import asyncio
import bleak
import numpy
import time
from PIL import Image, ImageOps

SERVICE_UUID = '0000af30-0000-1000-8000-00805f9b34fb'
CHARACTERISTIC_UUID = '0000ae01-0000-1000-8000-00805f9b34fb'

def convert_image(img: Image) -> Image:
    img_original = img.convert('RGBA')
    im = Image.new("RGB", img_original.size, (255, 255, 255))
    im.paste(img_original, mask=img_original.split()[3])
    wpercent = (384 / float(im.size[0]))
    hsize = int((float(im.size[1]) * float(wpercent)))
    im = im.resize((384, hsize), Image.ANTIALIAS)
    im = im.convert("L")
    im = ImageOps.invert(im)
    im = im.convert("1")
    return im

async def find():
   devices = await bleak.BleakScanner.discover()
   for d in devices:
     if SERVICE_UUID in d.metadata['uuids']:
         return d.address

async def write_bytes(printer, data):
    for chunk in (data[i:i+244] for i in range(0, len(data), 244)):
        await printer.write_gatt_char(CHARACTERISTIC_UUID, chunk, response=False)
        await asyncio.sleep(0.01)

def printer_crc(data):
    CRC8_TABLE = [
            ,
        ]

    crc = 0
    for b in data:
        crc = CRC8_TABLE[(crc ^ b)]

    return crc

async def write_command(printer, command: int, payload: bytes):
    data = bytearray([0x51, 0x78, command, 0x00, len(payload), 0x00])
    data.extend(payload)
    data.extend([printer_crc(payload), 0xff])
    await write_bytes(printer, data)

async def main():
    address = await find()
    if not address:
      print("Cannot find printer")
      return

    print(f"Connecting to printer at {address}")
    async with bleak.BleakClient(address) as client:
        print("Connected to printer")

        with Image.open("nyan-cat-01-625x450.jpg") as im:
          print(im.size)
          im = convert_image(im)
          print(im.size)
          pix = numpy.array(im, dtype=bool)
          for row in pix:
              im_bytes = bytes(numpy.packbits(row))
              await write_command(client, 0xa2, im_bytes)

          await write_command(client, 0xa1, [0x00, 0x30])

if __name__ == "__main__":
    loop = asyncio.get_event_loop()
    loop.run_until_complete(main())
    while True:
      time.sleep(1)
